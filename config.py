...
class Config:
    ...
    slack_alert_channel: str = field(
        default_factory=lambda: os.getenv("SLACK_ALERT_CHANNEL", "#incidents")
    )
    ...
    cpu_threshold_pct: float = field(
        default_factory=lambda: float(os.getenv("CPU_THRESHOLD_PCT", "85"))
    )
    memory_threshold_pct: float = field(
        default_factory=lambda: float(os.getenv("MEMORY_THRESHOLD_PCT", "90"))
    )
    slow_query_threshold_ms: int = field(
        default_factory=lambda: int(os.getenv("SLOW_QUERY_THRESHOLD_MS", "500"))
    )
    ...
    # When False (the default), EVERY incident waits for explicit human approval
    # before any action is taken — the safest posture. Set AUTO_REMEDIATE=true to
    # let the agent auto-execute fixes for non-critical, high-confidence incidents
    # while running in simulation mode (handy for demos and CI).
    auto_remediate: bool = field(
        default_factory=lambda: os.getenv("AUTO_REMEDIATE", "false").lower() == "true"
    )
    # Minimum self-critique confidence score (0-100) required before the agent is
    # allowed to act without a human. Low-confidence analyses always escalate.
    auto_approve_min_confidence: int = field(
        default_factory=lambda: int(os.getenv("AUTO_APPROVE_MIN_CONFIDENCE", "80"))
    )

    # ── Storage ─────────────────────────────────────────────────────────────
    db_path: str = field(
        default_factory=lambda: os.getenv("OPENSRE_DB_PATH", "opensre_incidents.db")
    )
