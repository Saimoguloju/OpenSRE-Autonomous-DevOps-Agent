...
FORBIDDEN_NAMESPACES = ["kube-system", "kube-public", "kube-node-lease", "kube-node", "security-system"]
...
class RemediationGuardrail:
    ...
    def validate_command(command: str) -> Tuple[bool, str]:
        ...
        if "kubectl" in cmd_lower:
            ...
            ns_match = re.search(r"(?:-n\s+|--namespace=|\s+--namespace\s+)([a-zA-Z0-9_-]+)", cmd_lower)
            if ns_match:
                ...
                if "delete" in cmd_lower and "all" in cmd_lower:
                    return False, "Kubectl delete all without explicit namespace is forbidden"

        # 4. Check for SQL Injection risks in database tool inputs
        if any(keyword in cmd_lower for keyword in ["select", "update", "delete", "insert"]):
